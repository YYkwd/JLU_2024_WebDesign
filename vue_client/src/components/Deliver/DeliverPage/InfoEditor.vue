<template>
  ...
  const avatarUpload = async (option: any) => {
    ...
    try {
      ...
      // 更新头像URL
      userInfo.value.avatarurl = response.data.data;  // 假设服务器返回的图片URL在 data 中
  ...
  const fetchUserInfo = async () => {
    try {
      const res = await api.get('/deliver/current', 
  ...
  const saveData = async () => {
    try {
      const response = await api.put('/deliver/update', userInfo.value ,
      {headers : { authorization : UserStore.authorization}}
